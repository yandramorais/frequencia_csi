...
class PulseFiModel(nn.Module):
    def __init__(self, input_dim, hidden_dim, n_layers):
        super(PulseFiModel, self).__init__()
...
def train():
    ...
    model = PulseFiModel(example_x.shape[2], HIDDEN_SIZE, NUM_LAYERS).to(DEVICE)
    criterion = nn.L1Loss()
